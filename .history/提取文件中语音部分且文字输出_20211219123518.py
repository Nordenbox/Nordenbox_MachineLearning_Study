...
count = 0
for i in pieces:
    i.export('/Users/nordenbox/Movies/voice_picese'+str(count)+'.wav',format='wav')
    count += 1
    
# 定义文件读取函数read_file()，传入参数文件地址filePath。
# 目的即读取语音碎片文件然后输出成一个打开好的音频文件。
...
# 通过for循环批量读取切分完的音频文件。
# 我觉得这么写的目的只在于 python 不能直接遍历文件夹。所以才有这么奇怪的写法。
# 我的理解是，前面的export 将一堆切割好语音碎片放到了与原始语音文件同一文件家里，不好遍历。
# 并且遍历之后，还是要一个个打开，不如用一个简单的代码直接来。
# 核心的秘密就是使用一个遍历去循环碎片文件的前缀。
