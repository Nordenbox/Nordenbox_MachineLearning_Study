...
class Sampling_balls():

    def __init__(self,ball_numbers):
        self.ball_number = ball_numbers

    def rendering_original_balls_set(self):
        balls = ['{}'.format(random.randint(0, 3)) for i in range(1, self.ball_numbers)] # create a random balls set
        balls_new = []
    ...
    def pick_up_sampling(self):
        ...
        while stop_pick:
            pick_times += 1
            pick_value = random.choice(self.rendering_original_balls_set())   # 每次在新的球的序列中提取一个小球
            # print(pick_value)
...
res = Sampling_balls(1000)
res.pick_up_sampling()